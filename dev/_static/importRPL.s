...
string depth_units_value,width_units_value,height_units_value
string imagename
...
if(textline.find("width-name")!=-1)
       { 
      width_name_value = textline.right(textline.len() - 10)
...
if(textline.find("width-units")!=-1)
       { 
      width_units_value = textline.right(textline.len() - 11)
      width_units_value = width_units_value.left(width_units_value.len()-2)
       }
...
if(textline.find("height-name")!=-1)
       { 
      height_name_value = textline.right(textline.len() - 11)
      height_name_value = height_name_value.left(height_name_value.len()-2)
       }
   
if(textline.find("height-units")!=-1)
       { 
      height_units_value = textline.right(textline.len() - 12)
      height_units_value = height_units_value.left(height_units_value.len()-2)
       }
...
if(textline.find("depth-units")!=-1)
       ...
       depth_units_value = textline.right(textline.len() - 12)      
       depth_units_value = depth_units_value.left(depth_units_value.len()-2)
...
if(!ImageIsValid(img))
{
result(" Unregonazed data-type value. Data-type formats are:signed, unsigned and float. Using float by default."+"\n")
img := RealImage(imagename, pixel_size, size_x, size_y, size_z)
}
...
img.ImageSetDimensionUnitString(0, width_units_value )  
}
...
img.ImageSetDimensionUnitString(1, height_units_value )  
}
...
img.ImageSetDimensionUnitString(2, depth_units_value )  
}
